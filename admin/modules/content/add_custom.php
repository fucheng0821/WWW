                    ...
                    <form class="layui-form" method="POST" id="content-form">
                        ...
                        <div class="layui-form-item">
                            <div class="layui-input-block">
                                ...
                                <button type="button" class="layui-btn" id="publish-to-platforms">📱 发布到平台</button>
...
<script src="../../assets/js/enhanced-image-uploader.js"></script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const publishToPlatformsBtn = document.getElementById('publish-to-platforms');
        if (publishToPlatformsBtn) {
            publishToPlatformsBtn.addEventListener('click', function() {
    ...
    // 发布内容到选定平台
    function publishContentToPlatforms() {
        // 验证基本信息
        ...
        // 显示加载提示
        window.layer.msg('正在获取可用平台...', {icon: 16, time: 0});
        
        // 获取已启用的平台列表
        fetch('/admin/modules/system/get_platforms.php')
            ...
            .then(result => {
                ...
                if (result.code === 0 && result.data && result.data.length > 0) {
                    ...
                    result.data.forEach(platform => {
                        // 检查平台配置是否完整
                        if (!platform.config_complete) {
                            platformOptions += `<label class="layui-checkbox layui-checkbox-disbaled">
                                <input type="checkbox" value="${platform.platform_key}" disabled>${platform.name}（配置不完整）
                            </label>`;
                        } else {
                            platformOptions += `<label class="layui-checkbox">
                                <input type="checkbox" value="${platform.platform_key}">${platform.name}
                            </label>`;
                        }
                    });
                    ...
                    const publishTypeOptions = `
                        <div class="layui-form-item">
                            ...
                            <div class="layui-input-block">
                                <select id="publish-type" class="layui-select">
                    ...
                    const scheduledTimePicker = `
                        <div class="layui-form-item" id="scheduled-time-container" style="display: none;">
                            <label class="layui-form-label">发布时间</label>
                            <div class="layui-input-block">
                                <input type="datetime-local" id="scheduled-time" class="layui-input">
                            </div>
                        </div>
                    `;
                    ...
                    window.layer.open({
                        ...
                        content: `
                            <div style="padding: 20px;">
                                <div class="layui-form-item">
                                    ...
                                    <div class="layui-input-block">
                                        <div id="platform-checkboxes" class="layui-form">
                                            ${platformOptions}
                                        </div>
                                    </div>
                                </div>
                        ...
                        success: function(layero, index) {
                            ...
                            // 监听发布类型变化
                            document.getElementById('publish-type').addEventListener('change', function() {
                        ...
                        yes: function(index, layero) {
                            ...
                            if (selectedPlatforms.length === 0) {
                                window.layer.msg('请至少选择一个平台', {icon: 2});
                                return;
                            }
                            
                            // 获取发布类型
                            const publishType = document.getElementById('publish-type').value;
                            ...
                            if (publishType === 'scheduled') {
                                ...
                                if (!scheduledTime) {
                                    window.layer.msg('请选择定时发布时间', {icon: 2});
                                    return;
                                }
                            }
                            ...
                            // 显示发布中提示
                            window.layer.msg('正在发布到平台...', {icon: 16, time: 0});
                            
                            // 执行发布
                            saveContentThenPublish(publishData);
                        }
                    });
                } else {
                    window.layer.msg(result.message || '暂无可用平台', {icon: 2});
                }
            })
            .catch(error => {
                console.error('获取平台列表失败:', error);
                window.layer.closeAll();
                window.layer.msg('获取平台列表失败，请重试', {icon: 2});
            });
    }
    
    // 保存内容然后发布
    function saveContentThenPublish(publishData) {
        // 模拟保存内容后获取内容ID
        // 实际应用中应先保存内容到数据库，获取内容ID后再发布
        // 这里使用临时ID作为示例
        ...
        const finalPublishData = {
            ...
            scheduled_time: publishData.scheduled_time
        };
        
        // 发送发布请求
        fetch('/admin/modules/system/publish_content.php', {
            method: 'POST',
            headers: {
        ...
        .then(result => {
            window.layer.closeAll();
            if (result.code === 0) {
                window.layer.msg('发布成功！', {icon: 1});
                // 可以添加跳转到发布日志页面的逻辑
            } else {
                window.layer.msg('发布失败：' + (result.message || '未知错误'), {icon: 2});
            }
        })
        .catch(error => {
            console.error('发布请求失败:', error);
            window.layer.closeAll();
            window.layer.msg('发布请求失败，请重试', {icon: 2});
        });
    }
