...
// 自动检测数据库字段名
function detectActualFieldNames($db) {
    $field_names = [
        'status_field' => 'is_active',  // 默认使用is_active
        'seo_prefix' => 'seo_'          // 默认使用seo_前缀
    ];
    
    try {
        // 检查categories表实际使用的状态字段
        $stmt = $db->query("SHOW COLUMNS FROM categories");
        $columns = $stmt->fetchAll();
        
        foreach ($columns as $col) {
            if (in_array($col['Field'], ['is_enabled', 'is_active'])) {
                $field_names['status_field'] = $col['Field'];
                break;
            }
        }
        
        // 检查contents表实际使用的SEO字段前缀
        $stmt = $db->query("SHOW COLUMNS FROM contents");
        $columns = $stmt->fetchAll();
        
        foreach ($columns as $col) {
            if (strpos($col['Field'], 'seo_title') !== false) {
                $field_names['seo_prefix'] = 'seo_';
                break;
            } elseif (strpos($col['Field'], 'meta_title') !== false) {
                $field_names['seo_prefix'] = 'meta_';
                break;
            }
        }
...
// 改进的slug生成函数，确保唯一性
function generateUniqueSlug($db, $base_title) {
    // 生成基础slug
    $base_slug = generate_slug($base_title);
    ...
    do {
        $stmt = $db->prepare("SELECT COUNT(*) FROM contents WHERE slug = ?");
        $stmt->execute([$slug]);
        ...
        if ($exists) {
            ...
            // 防止无限循环
            if ($counter > 100) {
                $slug = $base_slug . '-' . time() . '-' . rand(1000, 9999);
                break;
            }
        }
    } while ($exists);
...
// 获取实际字段名
$field_names = detectActualFieldNames($db);
$status_field = $field_names['status_field'];
$seo_prefix = $field_names['seo_prefix'];
        ...
        if (empty($leaf_categories)) {
            ...
            foreach ($leaf_categories as $category) {
                ...
                for ($i = 1; $i <= 10; $i++) {
                    ...
                    // 使用改进的slug生成函数确保唯一性
                    $slug = generateUniqueSlug($db, $title . '-' . time() . '-' . $i);
                    ...
                    $insert_sql = "
                        ...
                        (category_id, title, slug, summary, content, thumbnail, images, tags, 
                         ...
                         {$seo_prefix}title, {$seo_prefix}keywords, {$seo_prefix}description, created_at, updated_at) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
                    ";
...
try {
    ...
    $all_categories = [];
    $leaf_categories = [];
    $error_messages[] = "获取栏目数据失败：" . $e->getMessage();
}
?>
<!DOCTYPE html>
<html lang="zh-CN">
...
<body>
    <div class="container">
        ...
        <div class="diagnosis-info">
            <h4>🔍 数据库字段检测结果</h4>
            <p><strong>状态字段：</strong><span class="field-name"><?php echo $status_field; ?></span></p>
            <p><strong>SEO字段前缀：</strong><span class="field-name"><?php echo $seo_prefix; ?></span></p>
            <p><small>工具已自动适配您的数据库结构，避免字段名不匹配错误</small></p>
        </div>
        ...
        <div style="text-align: center; margin: 30px 0;">
            ...
            <a href="data_generator.php" class="layui-btn layui-btn-primary layui-btn-lg">🔙 返回工具入口</a>
    ...
    layui.use(['layer'], function(){
        ...
        window.onload = function() {
            <?php if (empty($error_messages) && !$success_messages): ?>
            layer.msg('✅ 工具已根据您的数据库结构自动适配', {icon: 1, time: 3000});
            <?php endif; ?>
        };
    });
    </script>
