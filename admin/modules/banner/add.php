...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $db->prepare("
                INSERT INTO banners (title, subtitle, image_url, link_url, banner_type, sort_order, is_active, created_at, updated_at) 
                VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
            ");
            $stmt->execute([$title, $subtitle, $image_url, $link_url, $banner_type, $sort_order, $is_active]);
                    ...
                    <form class="layui-form" method="POST">
                        ...
                        <div class="layui-form-item">
                            ...
                            <div class="layui-input-block">
                                ...
                                <!-- 实时预览区域 -->
                                <div class="image-preview" id="preview_image_url" style="margin-top: 10px;<?php echo empty($_POST['image_url'] ?? '') ? 'display:none;' : ''; ?>">
                                    <?php if (!empty($_POST['image_url'] ?? '')): ?>
                                        <img src="<?php echo htmlspecialchars($_POST['image_url'] ?? ''); ?>" style="max-width: 300px; max-height: 150px; border: 1px solid #eee; padding: 5px;">
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="layui-form-item">
                            ...
                            <div class="layui-input-block">
                                <select name="banner_type">
                                    <option value="home" <?php echo ($_POST['banner_type'] ?? 'home') === 'home' ? 'selected' : ''; ?>>首页Banner</option>
                                    <option value="inner" <?php echo ($_POST['banner_type'] ?? 'home') === 'inner' ? 'selected' : ''; ?>>内页Banner</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="layui-form-item">
                            ...
                            <div class="layui-input-block">
                                <input type="checkbox" name="is_active" lay-skin="switch" lay-text="启用|禁用" <?php echo !isset($_POST['is_active']) || $_POST['is_active'] ? 'checked' : ''; ?>>
                            </div>
                        </div>
                        
                        <div class="layui-form-item">
                            <div class="layui-input-block">
                                <button type="submit" class="layui-btn layui-btn-normal">添加Banner</button>
