...
if (empty($category_id)) {
    header("Location: index.php?error=" . urlencode('无效的栏目ID'));
    exit();
}

// 获取所有顶级栏目用于父级选择（排除当前栏目）
try {
    $stmt = $db->prepare("SELECT id, name FROM categories WHERE parent_id = 0 AND id != ? ORDER BY sort_order ASC, id DESC");
...
try {
    ...
    if (!$category) {
        header("Location: index.php?error=" . urlencode('栏目不存在'));
        exit();
    }
} catch(PDOException $e) {
    header("Location: index.php?error=" . urlencode('获取栏目信息失败'));
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        try {
            // 更新栏目
            $stmt = $db->prepare("UPDATE categories SET parent_id = ?, name = ?, description = ?, sort_order = ?, is_active = ?, updated_at = NOW() WHERE id = ?");
            $stmt->execute([$parent_id, $name, $description, $sort_order, $is_active, $category_id]);
            
            // 重定向到栏目列表页面
            header("Location: index.php?message=" . urlencode('栏目更新成功'));
            exit();
        } catch(PDOException $e) {
            $error_message = '更新栏目失败: ' . $e->getMessage();
                ...
                <form method="POST">
                    <div class="form-group">
                        ...
                        <select name="parent_id" class="form-control">
                            <option value="0" <?php echo ($category['parent_id'] == 0) ? 'selected' : ''; ?>>作为顶级栏目</option>
                    ...
                    <div class="form-group">
                        ...
                        <div class="custom-editor">
                            ...
                            <div id="custom-editor" class="editor-content" contenteditable="true" style="min-height: 150px; border: 1px solid #e6e6e6; padding: 15px; background: #fff; font-family: 'Microsoft YaHei', 'PingFang SC', sans-serif; font-size: 14px; line-height: 1.6;">
                                <?php echo $category['description'] ?? '<p>开始编写栏目描述...</p>'; ?>
                            </div>
                        </div>
                        <textarea name="description" id="description-input" style="display: none;"><?php echo htmlspecialchars($category['description'] ?? ''); ?></textarea>
                    </div>
                    
                    <div class="form-group">
                        <label>排序</label>
                        <input type="number" name="sort_order" class="form-control" value="<?php echo intval($category['sort_order'] ?? 0); ?>">
                    </div>
